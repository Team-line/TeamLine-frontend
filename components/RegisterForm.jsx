...
export const RegisterForm = ({ initialToken }) => {
  ...
  const data = [
    { id: 1, fieldKey: 'name', name: 'الاسم الكامل', place: 'ادخل اسمك الكامل', icon: faUser, type: "text" },
    { id: 2, fieldKey: 'email', name: 'البريد الإلكتروني', place: 'example@gmail.com', icon: faEnvelope, type: "email" },
    { id: 3, fieldKey: 'password', name: 'كلمة المرور', place: '••••••••', icon: faLock, type: 'password' },
  ]
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const receivedUser = response?.newUser || response.data?.newUser;
      if (receivedToken) {
    ...
    <div className='w-full flex flex-col justify-center items-center'>
      <div className='w-full max-w-md bg-white p-8 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-6'>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
          ...
          >
            {status.type === 'loading' ? 'جاري التسجيل...' : 'إنشاء حساب جديد'}
          </button>

            <h3 className='text-gray-500 mx-auto'>لديك حساب بالفعل؟ <Link href={'/Login'} className='text-[#3730a3] font-bold '>تسجيل الدخول</Link></h3>
