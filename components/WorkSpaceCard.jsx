...
export const WorkSpaceCard = ({ name,id,description }) => {
  ...
  return (
    <div 
      className="group p-5 relative rounded-2xl bg-white border border-gray-400 shadow-[0_2px_8px_-3px_rgba(0,0,0,0.05)] transition-all duration-300 hover:shadow-[0_10px_20px_-5px_rgba(0,0,0,0.08)] hover:border-slate-300 flex flex-col justify-between h-40 cursor-pointer"
      onClick={handleClick}
    >
      ...
      <div className='flex justify-between items-center'>
        ...
        {/* زر النقاط الثلاث */}
        <div 
          className="flex flex-col gap-1 p-2 cursor-pointer hover:bg-gray-100 rounded-full w-8 h-8 justify-center items-center transition-colors duration-200 z-10"
      ...
      {isOpen && (
        <div 
          className='absolute left-4 top-14 bg-white border border-slate-100 shadow-xl rounded-xl flex flex-col py-1.5 min-w-[120px] z-20 transition-all duration-200'
          onClick={(e) => e.stopPropagation()}  
        >
          <button 
            onClick={''}
            className='text-right px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 transition-colors'
          >
            تعديل
          </button>
          <button 
            onClick={handleDelete}
            className='text-right px-4 py-2 text-sm text-rose-600 hover:bg-rose-50 transition-colors'
          >
            حذف
          </button>
        </div>
